...
async function convertOggToMp3(buffer, outputFilePath) {
...
async function getTranscription(mimetype, data) {
  ...
  try {
    await convertOggToMp3(buffer, fileNameMp3);
    const transcript = await generateTranscription(fileNameMp3);
    return transcript || 'NO TRANSCRIPTION';
  } finally {
    try {
      await unlink(fileNameMp3);
    } catch (err) {
...
async function handleVoice({ mimetype, data }) {
  try {
    const transcription = await getTranscription(mimetype, data);
    return transcription;
  } catch (err) {
